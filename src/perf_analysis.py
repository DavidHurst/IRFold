...
for s_len in df.seq_length.unique():
    ...
    lv_distances = list(map(lv.ratio, s_len_df.our_shape_preds, s_len_df.rnalib_shape_preds))
...
plt.plot(df.seq_length.unique(), mean_lv_distances, label='Normalised Levenshtein Distance')
