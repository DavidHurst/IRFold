...
if __name__ == "__main__":
    ...
    for _ in range(2):
        ...
        wholly_nested_ir_pairs = [
            ...
            if irs_wholly_nested(pair[0], pair[1])
            or irs_wholly_nested(pair[0], pair[1])
        ]
        disjoint_ir_pairs = [
            pair
            for pair in separate_base_matching_ir_pairs
        ...
        for pair_type_bin_str, pair_type, pair_list in zip(
            ...
            for (ir_i, ir_j) in pair_list:
                (
                    ...
                    ir_i, ir_j, sequence_len, print_out=True
                )
                experiment_results["seq"].append(seq)
                experiment_results["seq_len"].append(sequence_len)
                experiment_results["num_valid_ir_gap_valid_base_pairing_pairs"].append(
                    len(separate_base_matching_ir_pairs)
                )
                experiment_results["ir_pair_index"].append(evaluated_pair_index)
